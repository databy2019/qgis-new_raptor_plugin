...
class NewRaptor:
    ...
    def run(self, lyr=None):
        ...
        if self.first_start == True:
            ...
            #QMessageBox.information(self.dlg, "Message", "Should run once first time run")

        #QMessageBox.information(self.dlg, "Message", "Should run every time")
        #panggil canvas dan ambil nilai titik kooordinat y dan x
        ...
        if result:
            ...
            #for linear layer bersinggungan
            ...
            QMessageBox.information(self.dlg, "Message", "New Nest ID: {}\n\n Latitude {}\n Longitude {}\n Species {}\n Buffer {}\n Status {}\n Last Survey {}\n".format(valNestID, valLat, valLng, valSpecies, valBuffer, valStatus, valLast))

            # save to layer
            ftrNests = QgsFeature(lyrNests.fields())

            ftrNests.setAttribute("lat_y_dd", valLat)
            ftrNests.setAttribute("long_x_dd", valLng)
            ftrNests.setAttribute("recentspec", valSpecies)
            ftrNests.setAttribute("buf_dist", valBuffer)
            ftrNests.setAttribute("recentstat", valStatus)
            ftrNests.setAttribute("lastsurvey", valLast)
            ftrNests.setAttribute("Nest_ID", valNestID)

            geom = QgsGeometry(QgsPoint(valLng, valLat))
            ftrNests.setGeometry(geom)
    ...
    def evt_cmbSpecies_changed(self, species):
        if species == "Swainsons Hawk":
            self.dlg.spbBuffer.setValue(0.00400)
        else:
            self.dlg.spbBuffer.setValue(0.00800)
